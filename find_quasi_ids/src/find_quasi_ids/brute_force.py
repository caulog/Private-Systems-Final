...
def compute_distinct_ratios_by_size(
    df, attribute_min=6, attribute_max=6, top_k=10, output_dir="output"
):
    """
    ...
    for r in range(attribute_min, attribute_max + 1):
        ...
        for combo in combinations(m, r):
            subset = df[
                list(combo)
            ]  # takes subset of dataframe with only current columns
            num_unique = len(
                subset.drop_duplicates()
            )  # removes duplicate rows to get unique row count
        ...
        for r_entry in top_results:
            print(
                f"{r_entry['attributes']}: distinct_ratio={r_entry['distinct_ratio']}, unique_combinations={r_entry['unique_combinations']}"
            )
